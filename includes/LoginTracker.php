...
class LoginTracker {
    ...
    public function isAccountLocked($email, $ipAddress, $deviceFingerprint) {
        try {
            ...
            return ['locked' => false, 'failed_count' => $failedCount];
        } catch (Exception $e) {
    ...
    public function recordFailedAttempt($email, $ipAddress, $deviceFingerprint) {
        try {
            // Check if there's already a record for this email
            ...
            if ($existing) {
                ...
                $this->db->update('failed_login_attempts', $data, 'email = ?', [$email]);
            } else {
                // Create new record
                $data = [
                    'email' => $email,
    ...
    public function isDeviceTrusted($userId, $deviceFingerprint) {
        try {
            ...
            $trustedDevice = $this->db->fetch($sql, [$userId, $deviceFingerprint]);
    ...
    public function addTrustedDevice($userId, $deviceFingerprint, $deviceName, $ipAddress, $userAgent, $locationData = null, $durationDays = 30) {
        try {
            $trustedUntil = date('Y-m-d H:i:s', strtotime("+{$durationDays} days"));
